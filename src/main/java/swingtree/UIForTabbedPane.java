...
public final class UIForTabbedPane<P extends JTabbedPane> extends UIForAnySwing<UIForTabbedPane<P>, P>
{
                            ...
                            if ( indexOfTab >= 0 && indexOfTab < tabCount )
                                _runInApp(() -> {
                                    try {
                                        ...
                                        log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab click!", ex);
                                    }
                                });
                        }
                    });
                            ...
                            if ( indexOfTab >= 0 && indexOfTab < tabCount )
                                _runInApp(() -> {
                                    try {
                                        ...
                                        log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab press!", ex);
                                    }
                                });
                        }
                    });
                            ...
                            if ( indexOfTab >= 0 && indexOfTab < tabCount )
                                _runInApp(() -> {
                                    try {
                                        ...
                                        log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab release!", ex);
                                    }
                                });
                        }
                    });
                            ...
                            if ( indexOfTab >= 0 && indexOfTab < tabCount )
                                _runInApp(() -> {
                                    try {
                                        ...
                                        log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab enter!", ex);
                                    }
                                });
                        }
                    });
                            ...
                            if ( indexOfTab >= 0 && indexOfTab < tabCount )
                                _runInApp(() -> {
                                    try {
                                        ...
                                        log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab exit!", ex);
                                    }
                                });
                        }
                    });
                       ...
                       if ( index >= 0 && index == tabbedPane.getSelectedIndex() )
                           _runInApp(()->{
                               try {
                                   ...
                                   log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab selection!", ex);
                               }
                           });
                   })
               );
    ...
    private <M> void _updateTabs(P pane, ValsDelegate<M> delegate, TabSupplier<M> tabSupplier) {
        ...
        switch (delegate.change()) {
            ...
            case NONE:
                break;
            default:
                log.warn(SwingTree.get().loggingMarker(), "Unknown change type: {}", delegate.change(), new Throwable());
        ...
        if ( diff == null || ( lastDiff == null || !diff.isDirectSuccessorOf(lastDiff) ) ) {
            ...
            switch (diff.change()) {
                ...
                case NONE:
                    break;
                default:
                    log.warn(SwingTree.get().loggingMarker(), "Unknown change type: {}", diff.change(), new Throwable());
                    // We do a simple rebuild:
                    pane.removeAll();
                    tupleOfModels.forEach(value -> _addTabAt(pane.getTabCount(), value, tabSupplier, pane));
            }
        ...
        return _with( thisComponent -> {
                    _onChange(thisComponent, e -> _runInApp(()->{
                        try {
                            ...
                            log.error(SwingTree.get().loggingMarker(), "Error while executing action on tab change!", ex);
                        }
                    }));
                })
                ._this();
    ...
    private <M> Tab _createTab( @Nullable M m, TabSupplier<M> tabSupplier ) {
        ...
        try {
            ...
            if ( tab == null ) {
                log.warn(SwingTree.get().loggingMarker(), "Tab supplier returned null for '{}'.", m, new Throwable());
                return UIForTabbedPane.TAB_NULL;
            }
            return tab;
        } catch (Exception e) {
            log.error(SwingTree.get().loggingMarker(), "Error while creating tab for '{}'.", m, e);
