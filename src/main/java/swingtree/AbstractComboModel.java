...
abstract class AbstractComboModel<E extends @Nullable Object> implements ComboBoxModel<E>
{
    ...
    void fireListeners() {
		try {
			...
			log.error(SwingTree.get().logMarker(), "An exception occurred while firing combo box model listeners!", e);
		}
    }
    ...
    void doQuietly( Runnable task ) {
		...
		try {
			...
			log.error(SwingTree.get().logMarker(), "An exception occurred while running a combo box model task!", e);
		}
	...
	private @Nullable E _convert( String o ) {
		...
		if ( type.isEnum() ) {
			...
			try {
				...
				log.debug(SwingTree.get().logMarker(), "Failed to parse enum string '"+name+"' as "+type+".", ignored);
			}
