...
public abstract class UIForAnything<I, C extends E, E extends Component>
{
    ...
    public final I peek( Peeker<C> action ) {
        return _with( thisComponent -> {
                    try {
                        ...
                        log.error(SwingTree.get().loggingMarker(), "Error trying to run 'Peeker' '"+action+"' on component '"+thisComponent+"'.", e);
                    }
                })
                ._this();
    }
    ...
    private <T> void _onShow(
        ...
        if (!(component instanceof JComponent)) {
            log.error(SwingTree.get().loggingMarker(), "Invalid internal state detected! The component wrapped by this builder is null or not a JComponent. ", new Throwable());
            return;
        }
