...
final class StyleInstaller<C extends JComponent>
{
        ...
        if ( !isStyled || !weNeedToInstallTheCustomUI ) {
            ...
            if ( !isStyled ) {
                ...
                if ( owner instanceof JLabel ) {
                    ...
                    String stripped = _stripHtmlInjection(label.getText());
    ...
    */
    ...
    private static void _applyFontConfToHtmlLabel(JLabel label, FontConf fontConf) {
        ...
        String original = _stripHtmlInjection(currentText);
        String css = _buildHtmlBodyCss(fontConf);
        String desiredText = css.isEmpty() ? original : _injectStyleTag(original, css);
    ...
    /*
        ...
        for non-HTML text.
    */
    private static void _ensureHtmlTextListenerInstalled(JLabel label) {
        ...
        label.addPropertyChangeListener("text", evt -> {
            ...
            if ( !BasicHTML.isHTMLString(newText) )
                return;
            if ( newText.length() >= _HTML_OPEN_TAG_LEN
    ...
    private static String _stripHtmlInjection(String html) {
    ...
    private static String _injectStyleTag(String html, String css) {
        if ( html.length() < _HTML_OPEN_TAG_LEN )
            return html;
        return html.substring(0, _HTML_OPEN_TAG_LEN)
    ...
    private static String _buildHtmlBodyCss(FontConf fontConf) {
