...
final class StyleSource<C extends JComponent>
{
    ...
    StyleConf gatherStyleFor(C owner )
    {
        ...
        try {
            ...
            log.error(SwingTree.get().loggingMarker(), "An exception occurred while applying the style sheet for component '"+owner+"'.", e);
            /*
        ...
        try {
            ...
            log.error(SwingTree.get().loggingMarker(), "An exception occurred while applying the local styler for component '"+owner+"'.", e);
            /*
        ...
        for ( Expirable<Styler<C>> expirableStyler : _animationStylers )
            try {
                ...
                log.warn(SwingTree.get().loggingMarker(), "An exception occurred while applying an animation styler!", e);
                /*
                     If any exceptions happen in a Styler implementation provided by a user,
                     then we don't want to prevent the other Stylers from doing their job,
                     which is why we catch any exceptions immediately!
