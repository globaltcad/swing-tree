...
final class StyleRenderer
{
    ...
    static @Nullable Paint _createGradientPaint(
        ...
        if ( type.isOneOf(UI.Span.TOP_LEFT_TO_BOTTOM_RIGHT) ) {
            ...
            log.warn(SwingTree.get().loggingMarker(), "Unknown gradient type: " + type, new Throwable());
            return null;
        }
    ...
    private static MultipleGradientPaint.CycleMethod _cycleMethodFrom(UI.Cycle cycle) {
        switch (cycle) {
            ...
            default:
                log.warn(SwingTree.get().loggingMarker(), "Unknown cycle method: " + cycle, new Throwable());
    ...
    static void renderParentFilter(
        ...
        try {
            ...
            log.error(SwingTree.get().loggingMarker(), "Failed to successfully render filtered parent buffer!", e);
        } finally {
            g2d.setClip(oldClip);
        }
    }
