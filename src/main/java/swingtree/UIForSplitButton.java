            ...
            thisComponent.addButtonClickedActionListener(e -> _runInApp(()->{
                ...
                for ( JMenuItem item : selected ) {
                    ...
                    try {
                        ...
                        log.error(SwingTree.get().logMarker(), "Error while executing split button action listener.", exception);
                    }
                }
            }));
        });
    ...
    private static <E extends Enum<E>> Function<E, String> _exceptionSafeTextProvider( Function<E, String> textProvider ) {
        return e -> {
            try {
                ...
                log.error(SwingTree.get().logMarker(), "Error while providing split button text for enum value.", ex);
            }
            try {
                return e.toString();
            } catch (Exception ex) {
                log.error(SwingTree.get().logMarker(), "Error while providing split button text for enum value using 'toString()'.", ex);
            }
            try {
                return e.name();
            } catch (Exception ex) {
                log.error(SwingTree.get().logMarker(), "Error while providing split button text for enum value using 'name()'.", ex);
            }
            return "";
        };
    }
                    ...
                    thisComponent.addButtonClickedActionListener(
                        e -> _runInApp(()->{
                            try {
                                ...
                                log.error(SwingTree.get().logMarker(), "Error while executing action on button click!", ex);
                            }
                        })
                    )
                )
            ...
            e -> _runInApp(()->{
                ...
                state.onSelections.forEach(action -> {
                    try {
                        ...
                        log.error(SwingTree.get().logMarker(), "Error while executing selection action listener.", exception);
                    }
                });
                try {
                    splitItem.getOnSelected().accept(delegate);
                } catch (Exception exception) {
                    log.error(SwingTree.get().logMarker(), "Error while executing split item selection action.", exception);
                }
            })
        );
    }
