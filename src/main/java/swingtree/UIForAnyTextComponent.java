                        ...
                        @Override public void insertUpdate(DocumentEvent e)  {
                            _runInApp(()->{
                                try {
                                    ...
                                    log.error(SwingTree.get().loggingMarker(), "Error while executing action on text component content change!", ex);
                                }
                            });
                        }
        ...
        return _with( thisComponent -> {
                    _onTextChange(thisComponent, e -> _runInApp( () -> {
                        try {
                            ...
                            log.error(SwingTree.get().loggingMarker(), "Error while executing action on text change!", ex);
                        }
                    }));
                })
                ._this();
                ...
                public void remove(FilterBypass fb, int offset, int length) throws BadLocationException {
                    state.removes.forEach(action -> {
                        try {
                            ...
                            log.error(SwingTree.get().loggingMarker(), "Error while executing action on text remove!", e);
                        }
                    });
                ...
                public void insertString(FilterBypass fb, int offset, String string, AttributeSet attr) throws BadLocationException {
                    state.inserts.forEach(action -> {
                        try {
                            ...
                            log.error(SwingTree.get().loggingMarker(), "Error while executing action on text insert!", e);
                        }
                    });
                ...
                public void replace(FilterBypass fb, int offset, int length, @Nullable String text, AttributeSet attrs) throws BadLocationException {
                    state.replaces.forEach(action -> {
                        try {
                            ...
                            log.error(SwingTree.get().loggingMarker(), "Error while executing action on text replace!", e);
                        }
                    });
