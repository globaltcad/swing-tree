...
public abstract class UIFactoryMethods extends UILayoutConstants
{
    ...
    public static UI.Font font( String fontString ) {
        ...
        if ( font == null ) {
            if ( potentialProblem1 != null )
                log.error(SwingTree.get().loggingMarker(), "Could not parse font string '" + fontString + "' using 'Font.decode(String)'.", potentialProblem1);
            if ( potentialProblem2 != null )
                log.error(SwingTree.get().loggingMarker(), "Could not parse font string '" + fontString + "' from 'System.getProperty(String)'.", potentialProblem2);

            log.error(SwingTree.get().loggingMarker(), "Could not parse font string '" + fontString + "' using 'Font.decode(String)' or 'System.getProperty(String)'.", new Throwable());

            try {
                return UI.Font.of(fontString, UI.FontStyle.PLAIN, UI.scale(12));
            } catch (Exception e) {
                log.error(SwingTree.get().loggingMarker(), "Could not create font with name '" + fontString + "' and size 12.", e);
    ...
    public static UIForTable<JTable> table(
        ...
        try {
            ...
            log.error(SwingTree.get().loggingMarker(), "Failed to configure table model!", e);
            return table();
        }
    ...
    public static void showUsing(EventProcessor eventProcessor, Function<JFrame, java.awt.Component> uiSupplier ) {
        ...
        show(frame -> use(eventProcessor, () -> {
            try {
                ...
                log.error(SwingTree.get().loggingMarker(), "Error trying to create a UI component for a new JFrame.", e);
                return panel("fill")
        ...
        private TestWindow( String title, Function<JFrame, java.awt.Component> uiSupplier ) {
            ...
            if ( !UI.thisIsUIThread() ) {
                try {
                    ...
                    log.error(SwingTree.get().loggingMarker(), "Error trying to create UI component for display in new JFrame, using supplier function '{}'.", uiSupplier, e);
                }
            }
            else
    ...
    private static @Nullable ImageIcon _tryLoadIcon(IconDeclaration declaration )
    {
        ...
        try {
            ...
            log.error(SwingTree.get().loggingMarker(), "Failed to load icon from declaration: " + declaration, e);
        }
        return icon;
    }
    ...
    private static @Nullable ImageIcon _loadIcon( IconDeclaration declaration )
    {
        ...
        if ( path.endsWith(".svg") ) {
            ...
            try {
                ...
                log.error(SwingTree.get().loggingMarker(), "Failed to load SVG document from URL: " + url, e);
                return null;
            }
