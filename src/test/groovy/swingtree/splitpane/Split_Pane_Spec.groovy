...
class Split_Pane_Spec extends Specification
{
    ...
    def 'A split pane can be configured with a divider size.'()
    {
        given : 'We create a split pane UI node with a divider size of 10.'
    ...
    def 'A horizontally aligned split pane can be configured with a divider location as a percentage.'()
    {
        given : 'We create a split pane UI node with a division of 50%.'
            var ui = UI.splitPane(UI.Align.HORIZONTAL)
                        .withWidth(42)
                        .withHeight(100)
                        .withDivisionOf(0.5)
        ...
        and : 'The split pane is horizontally aligned, meaning it splits vertically.'
            ui.component.orientation == JSplitPane.VERTICAL_SPLIT
        and : 'The divider location is 50.'
            ui.component.dividerLocation == 50
    }

    def 'A vertically aligned split pane can be configured with a divider location as a percentage.'()
    {
        given : 'We create a split pane UI node with a division of 50%.'
            var ui = UI.splitPane(UI.Align.VERTICAL)
                        .withWidth(100)
    ...
    def 'The division of a split pane can dynamically be configured through a double property.'()
    {
        given : 'We create a simple view model property holding the division of our split pane.'
        ...
        then : 'The divider location is 25.'
            ui.component.dividerLocation == 25

        when : 'We change the division property to 0.75.'
            divisionPercentage.set(0.75d)
            UI.sync()
        then : 'The divider location is 75.'
            ui.component.dividerLocation == 75
    }

    def 'You can dynamically model the divider size of a split pane in your view model.'()
    {
        reportInfo """
            ...
            registered on the property.
        """
        given : 'We create a simple view model property holding the divider size of our split pane.'
