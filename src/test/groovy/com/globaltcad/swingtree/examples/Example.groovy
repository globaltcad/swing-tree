...
var ini =
(JFrame frame, pic, descr, explain) ->
                ...
                UI.scrollPane().add(
                    ...
                    .add("push, grow, span, wrap",
                        ...
                        .add("height 50:150:200, width 50:550:550, grow, push",
                           ...
                           .add(UI.editorPane().withForeground(Color.DARK_GRAY).withText((explain.strip().replace("\n", " "))).isEditableIf(false)))
                    )
                    ...
                    .apply( p -> {
                        pic.size().times {
                            i -> p.add(UI.tab("APIC$i").add(
                                UI.panel("fill")
                                .apply(inner -> {
                                    descr.keySet().each {label -> {
                                        var text = pic[i][label]
                                        ...
                                        .add("align right, shrink, wrap",
                                            UI.button("Help").withProperty("JButton.buttonType", "help")
                                            .onClick({JOptionPane.showMessageDialog(frame, descr[label])})
                                        )
                                    }}
                                })
                            ))
...
descr = [
        ...
        'SC':"<html><b>SC - Secure Containment</b> <br> Setjim bi mae contained or altered bitus on es  varitus<br> APIC laval medresan lower laval medres fo eti recusandae statute type.</html>",
...
new UI.TestWindow(()->frame,ini(frame, pic, descr, explain)).getFrame().setSize(new Dimension(1100, 600))
