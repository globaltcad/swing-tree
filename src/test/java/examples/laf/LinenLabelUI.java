 ...
 *  Working out where a label's text and icon go is more expensive than it
 *  looks: it has to measure the text, and measuring a string that contains a
 *  character the font cannot advance from a lookup table costs a hundred times
 *  what a plain ASCII string costs, because the toolkit falls back to shaping
 *  the whole line. Swing asks that question again on every single paint, even
 *  though a label's text, font, icon and size almost never change between two
 *  paints. This delegate therefore remembers the last few answers and hands
 *  one back whenever every input to it is unchanged — see {@link #layoutCL}.
 *  <p>
...
{
    /**
     *  How many placements one label remembers. Swing asks {@link #layoutCL} the same
     *  question from three directions: where the text sits at the size the label is
     *  painted at, how large the label would like to be (asked with an unbounded
     *  viewing rectangle), and where its baseline is (asked by the layout manager at
     *  whatever size it is currently considering, which is not always the size the
     *  label has). Remembering fewer than three answers makes those questions evict
     *  each other: a label answering all three in a loop hits 33% of the time with one
     *  slot and 50% with two, against 99.7% with three. A fourth slot buys nothing,
     *  because a label needing more than three answers is one whose content genuinely
     *  varies - a cell renderer standing in for many rows, say - and for those the
     *  answer has to be worked out anyway.
     */
    ...
    /**
     *  Places the label's text and icon inside {@code viewR}, filling {@code iconR}
     *  and {@code textR} and returning the text as it should be drawn — shortened
     *  with an ellipsis when it does not fit.
     *  <p>
     *  This is the single point through which {@link BasicLabelUI} reaches Swing's
     *  compound-label layout, for painting as well as for reporting a preferred size
     *  or a baseline, which is why it is the one place worth remembering. Every input
     *  the layout reads is part of what is compared here, so a label that changes any
     *  of them — its text, font, icon, alignment, gap, reading direction, or the
     *  rectangle it is placed in — simply does not find a match and is laid out
     *  afresh. No invalidation is needed anywhere else, and no listener has to fire
     *  for the answer to stay correct.
     *  <p>
     *  Markup is the one thing not remembered. Swing lays a label's markup out
     *  through a document view it builds and hangs off the label. That view keeps its
     *  own measurements and throws them away itself when it has to — when an image
     *  inside it finishes loading, say — without anything on the label changing.
     *  Remembering a placement for it would therefore shadow an invalidation this
     *  delegate cannot see, in exchange for a measurement that has already been made.
     */
    @Override
    ...
    /**
     *  One remembered answer of {@link #layoutCL}: everything the compound-label
     *  layout read to produce it, next to the three results it produced. Of an icon
     *  only the size it reported is held: the placement is worked out from that alone,
     *  and an icon that loads or animates reports a new size without becoming a new
     *  icon, so the size is both the necessary and the sufficient thing to compare.
     */
