...
public class NoteGuesserView extends Panel
{
    ...
    public NoteGuesserView(Var<NoteGuesserViewModel> vm) {
        ...
        .add(ALIGN_CENTER,
            ...
            .add(
                button("New Note").isEnabledIf(playMode.view( b -> !b ))
                .onClick( e -> {
                    vm.update(NoteGuesserViewModel::newRandomNoteIndex);
                })
            )
            .add(
                toggleButton("Just Play!", playMode)
            )
            .add(RIGHT,
                panel(INS(0, 12, 0, 12), "[]12[]")
                .add(
                    label(score.viewAsString( s -> "Score: " + s ))
                    .withFontSize(12)
                    .isEnabledIf(playMode.view( b -> !b )))
                .add(
                    label(level.viewAsString( l -> "Level: " + l ))
                    .withFontSize(12)
                    .isEnabledIf(playMode.view( b -> !b ))
                )
            )
    ...
    void buildKeyboard(UIForPanel<JPanel> p, Var<NoteGuesserViewModel> vm) {
        ...
        p.withStyle( it -> it
            .painter(Layer.BACKGROUND, g2d -> {
                ...
                whiteNotes.clear();
                blackNotes.clear();
                int width  = p.get(JPanel.class).getWidth();
                int height = p.get(JPanel.class).getHeight();
                int x = 0;
                int y = - ( roundness / 2 );
                ...
                vm.get().getOctaves().forEach(octave ->
                    octave.forEach( note -> {
                        int ni = note.index();
                        int oi = note.octave();
                        ...
                        int keyWidth  = width / vm.get().numWhiteNotes();
                ...
                whiteNotes.forEach( key -> {
                    boolean isCheating = vm.get().playMode();

                    // Finally we draw all the note labels on the left side of the scale:
                    int ni = key.note.index();
                    String note = vm.get().noteNameOf(ni);
                    if ( note != null ) {
                        boolean shouldDrawHelp = vm.get().shouldShowHelpFor(ni) || isCheating;
                        if ( shouldDrawHelp ) {
    ...
    void noteLines(
        ...
        Color color = vm.playMode() ? Color.LIGHTGRAY : Color.BLACK;
        ...
        for ( int ni = 0; ni < (vm.numWhiteNotes()); ni++ ) {
            ...
            if ( !isActuallyALine ) {
                float alpha = (float) ( 0.175f / Math.pow(2,vm.level()) );
