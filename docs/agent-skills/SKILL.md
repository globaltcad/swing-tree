...
> **CRITICAL: item types in a `Tuple` bound via `addAll` MUST implement
> `sprouts.HasId<IdType>`** (carry a `UUID`/stable id). Value records define
> identity by *content*, so two equal records would confuse the component
> binding — `HasId.id()` gives each item a stable identity so SwingTree knows
> which sub-view maps to which item. Add a `UUID id` field and
...
`addAll(list, viewSupplier)`. **TeamView exists in this repo in both flavors**
...
2. **`Tuple` items rendered via `addAll` must `implement HasId<UUID>`** with a stable
   id — otherwise equal value-records collide and bindings target the wrong sub-view.
...
panel.addAll(items, (Var<Item> it) -> itemView(it));      // Item implements HasId<UUID>!
