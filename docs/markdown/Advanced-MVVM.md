...
> **Prerequisites:** This guide builds on
> [Basic MVVM](./Basic-MVVM.md) — make sure you understand single-property
> bindings before tackling sub-view models and observable lists here.
>
...
The core principle of MVVM is the usage of so-called *properties*
...
- If you want to take it one step further and get rid of mutable view models
  entirely, the [MVI / MVL](./Functional-MVVM.md) guide shows how to model
  the whole UI state as a single immutable record, with **property lenses**
  generated by `Var.zoomTo(..)`.
- For a deeper rationale on *why* the functional approach scales so well,
  see the [Data Oriented SwingTree](./Data-Oriented-SwingTree.md) guide.
- The [Modelling Animations](./Modelling-Animations.md) guide complements MVVM
  by showing how to encode animation state itself into the view model.
